...
# ─── HELPERS ─────────────────────────────────────────────────────────────────
def compute_sb_overrides(beam_maj_as, beam_min_as,
                         target_ring_beam_flux=75e-6,  # Jy/beam
                         center_to_ring_SB_ratio=2.0,
                         center_diam_as=None, k_geom=1.0):
    """
    Return overrides for sky_config so every config lands at the same per-beam S/N.
    """
    # Beam area
    omega = 1.133 * beam_maj_as * beam_min_as  # arcsec^2

    # Uniform ring SB to hit target per-beam flux
    sb_ring = target_ring_beam_flux / omega  # Jy/arcsec^2

    # Center SB and total flux (needs center diameter)
    sb_center = center_to_ring_SB_ratio * sb_ring
    if center_diam_as is None:
        center_diam_as = k_geom * max(beam_maj_as, beam_min_as)
    area_center = math.pi * (0.5 * center_diam_as)**2
    central_flux = sb_center * area_center  # Jy

    # Geometry scaled with beam
    ring_thickness = k_geom * max(beam_maj_as, beam_min_as)
    ring_spacing   = 2.0 * max(beam_maj_as, beam_min_as)

    return {
        "ring_surface_brightness": sb_ring,
        "central_flux": central_flux,
        "central_diameter": center_diam_as,
...
def compute_baselines(ants):
    ...
    Parameters:
    ants (list of tuples): Antenna positions (x, y, z).
    
    Returns:
    np.array: Sorted array of baseline lengths in meters.
    """
    baselines = []
    n = len(ants)
    for i in range(n):
...
def compute_beam_fwhm(lam, L80):
    """
    Compute angular resolution (beam FWHM) from L80 baseline.
    
    Parameters:
    lam (float): Wavelength in meters.
    L80 (float): 80th percentile baseline in meters.
    
    Returns:
    float: Angular resolution in arcsec.
    """
    theta_rad = 0.574 * lam / L80
    return theta_rad * (180 / pi) * 3600  # convert to arcsec
    ...
    try:
        ...
        for dec in args.declinations:
            for config in config_files:
                ...
                beam = compute_beam_fwhm(lam, L80)  # arcsec
                logging.info(f"[{config_tag}] Estimated beam FWHM ≈ {beam:.4f} arcsec")

                # Pixel/cell and image size 
                dyn_cell = f"{beam / args.sampling_factor}arcsec"
                logging.info(f"Cell size set to {dyn_cell}")
                ...
                # Build per-config overrides if using the RING generator 
                use_ring = "ring" in Path(args.sky_script).name.lower()
                overrides = None
                suffix = config_tag  # keep files unique per configuration
                if use_ring:
                    overrides = compute_sb_overrides(
                        beam_maj_as=beam,
                        beam_min_as=beam,
                        target_ring_beam_flux=75e-6,   # e.g. 50 µJy/beam per ring
                        center_to_ring_SB_ratio=2.0,   # center 2x brighter per area
                        k_geom=1.0                      # central_diameter=1×beam, thickness=1×beam, spacing=2×beam
                    )

                # Generate the sky model NOW with overrides 
                fitsname = generate_sky_model(
                    dec,
                    args.sky_script,
                    args.sky_config,
                    args.casa_bin,
                ...
                fits_path = root_dir / fitsname
                fits_name = fits_path.stem

                # Simulate + image for each integration time
                    ...
                    for wt in args.weightings:
                        ...
                        for rb in robusts:
                            for tap in args.uvtaper_values:
                                ...
                                print(f'BEAM MAJOR FROM PSF: {beam_major}')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        parser.add_argument('--sky_config', default='sky_config.json',
                    help="Path to sky-model JSON config (e.g., sky_config.json or sky_config_ring.json)")
