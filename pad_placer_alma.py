...
def _query_open_elevation(latlon_list, batch_size=100, url="https://api.open-elevation.com/api/v1/lookup", timeout=30):
    """Return a list of elevations (m) for [(lat, lon), ...]."""
    elevations = []
    for i in range(0, len(latlon_list), batch_size):
        chunk = latlon_list[i:i+batch_size]
        locs = "|".join(f"{lat:.7f},{lon:.7f}" for lat, lon in chunk)
        r = requests.get(url, params={"locations": locs}, timeout=timeout)
        r.raise_for_status()
        data = r.json().get("results", [])
        elevations.extend([row["elevation"] for row in data])
    if len(elevations) != len(latlon_list):
        raise RuntimeError("Open-Elevation returned unexpected number of results.")
    return elevations

def add_open_elevation_z(new_df, crs_loc, lat0, lon0, relative_to_origin=True):
    """
    Fill new_df['z'] using Open-Elevation.
    If relative_to_origin=True, set z = elev(pad) - elev(origin) to express z in LOC.
    """
    # Transform LOC -> WGS84
    transformer = Transformer.from_crs(crs_loc, "EPSG:4326", always_xy=True)
    lons, lats = transformer.transform(new_df["x"].to_numpy(), new_df["y"].to_numpy())
    pad_elevs = _query_open_elevation(list(zip(lats, lons)))

    if relative_to_origin:
        origin_elev = _query_open_elevation([(lat0, lon0)])[0]
        z_vals = np.array(pad_elevs, dtype=float) - float(origin_elev)
    else:
        z_vals = np.array(pad_elevs, dtype=float)
...
def load_data(args):
    ...
    # CRS setup
    lat0, lon0 = -23.02271113, -67.75436287
    ...
    logging.info(f"Loaded {len(region_df)} region pads")
    ...
    return existing_df, existing_pts, ex_weights, ctr_start, allowed_area, region_df, crs_loc, lat0, lon0
...
def optimize_pads(args, existing_df, existing_pts, ex_weights, ctr_start, allowed_area, region_df):
    ...
    # Radial boosts (if wanted)
    #radial_boost = np.ones(len(cand_pts))
    #mask_inner = cand_rad < 2000
    #radial_boost[mask_inner] = 1
    #mask_mid1 = (cand_rad > 1500) & (cand_rad < 2500)
    #radial_boost[mask_mid1] = 0.8
    #mask_mid = (cand_rad > 5000) & (cand_rad < 11000)
    #radial_boost[mask_mid] = 0.8
    #mask_outer = cand_rad > 20000
    #radial_boost[mask_outer] = 0.3
    ...
    sel_df['z'] = 0.0
    sel_df['diam'] = 12.0
    new_df = sel_df[['name', 'x', 'y', 'z', 'diam']].copy()
...
def write_outputs(args, new_df, existing_df):
    ...
    with open(args.out_plus, 'w') as fh:
        ...
        for _, row in new_df.iterrows():
            fh.write(f"{row.x:.3f} {row.y:.3f} {row.z:.3f} {row.diam:.1f} {row['name']}\n")
    logging.info(f"Wrote combined config to {args.out_plus}")
...
def main():
    ...
    parser.add_argument('--cfg_path', default="alma.cycle11.10.cfg")
    parser.add_argument('--kml_path', default="RegionsOSF.kml")
    parser.add_argument('--poly_layers', nargs='+', default=['Regions OSF'], help="List of KML layers for allowed areas")
    parser.add_argument('--regions_kml', default="PadsAOS.kml")
    parser.add_argument('--out_new', default="new_pads_57.cfg")
    parser.add_argument('--out_plus', default="alma.cycle11.10.test.cfg")
    parser.add_argument('--n_new', type=int, default=27)
    parser.add_argument('--spacing', type=float, default=75.0)
    parser.add_argument('--min_bl', type=float, default=270)
    parser.add_argument('--R_max', type=float, default=30000)
    parser.add_argument('--inner_radius', type=float, default=4700)
    parser.add_argument('--favored_weight', type=float, default=0.75)
    parser.add_argument('--include_reuse', action='store_true', default=False)
    parser.add_argument('--cable_weight', type=float, default=5e-6)
    parser.add_argument('--inner_ratio', type=float, default=3)
    parser.add_argument('--outer_ratio', type=float, default=1)
    parser.add_argument('--fixed_r_max', type=float, default=60000.0)
    parser.add_argument('--ratio_penalty_weight', type=float, default=0)
    parser.add_argument('--transition_width', type=float, default=6000.0)
    parser.add_argument('--density_radius', type=float, default=1900.0)
    parser.add_argument('--density_weight', type=float, default=0.4)
    parser.add_argument('--sigma_b', type=float, default=24000, help="Sigma for Gaussian target distribution")
    parser.add_argument('--use_open_elevation', action='store_true', default=False,
                    help="Query Open-Elevation to set z for new pads.")
    parser.add_argument('--absolute_z', action='store_true', default=False,
                    help="If set, z will be absolute elevation (m). Default is LOC-relative (pad - origin).")

    args = parser.parse_args()

    existing_df, existing_pts, ex_weights, ctr_start, allowed_area, region_df, crs_loc, lat0, lon0 = load_data(args)
    ...
    if args.use_open_elevation:
        new_df = add_open_elevation_z(
            new_df, crs_loc, lat0, lon0,
            relative_to_origin=not args.absolute_z
        )
    write_outputs(args, new_df, existing_df)
    generate_plots(new_df, existing_df, fixed_r_max)

if __name__ == "__main__":
    # To run via command line, uncomment the following:
    # main()

    # Example run with hardcoded arguments
    example_args = argparse.Namespace(
        ...
        out_plus="alma.cycle11.10.test.cfg",
        n_new=30,
        spacing=25.0,
        min_bl=200,
        R_max=30000,
        ...
        outer_ratio=1,
        fixed_r_max=52000.0,
        ratio_penalty_weight=0,
        transition_width=6000.0,
        density_radius=1900.0,
        density_weight=0.4,
        sigma_b=24000,
    ...
    if example_args.use_open_elevation:
        new_df = add_open_elevation_z(
            new_df, crs_loc, lat0, lon0,
            relative_to_origin=not example_args.absolute_z
        )
